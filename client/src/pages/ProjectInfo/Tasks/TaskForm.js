...
function TaskForm({
    ...
    try {
      ...
      if (response.success) {
        if (!task) {
          ...
          AddNotification({
            title: `Você recebeu uma nova tarefa no ${project.name}`,
  ...
  return (
    <Modal
      title={task ? "Editar Tarefa" : "Criar Tarefa"}
      open={showTaskForm}
    ...
    >
      <Tabs activeKey={selectedTab} onChange={(key) => setSelectedTab(key)}>
        <Tabs.TabPane tab="Detalhes da tarefa" key="1">
          ...
          >
            <Form.Item label="Nome da tarefa" name="name">
              <Input />
            </Form.Item>

            <Form.Item label="Descrição da tarefa" name="description">
              <TextArea />
            </Form.Item>

            <Form.Item label="Atribuir a" name="assignedTo">
              <Input
            ...
            {email && !validateEmail() && (
              <div className="bg-red-700 text-sm p-2 rounded">
                <span className="text-white">
                  O e-mail não é válido ou o funcionário não está no projeto
                </span>
              </div>
            )}
          </Form>
        </Tabs.TabPane>
        <Tabs.TabPane tab="Anexos" key="2" disabled={!task}>
